...
usage="\nProgram:\thafoe
\nVersion:\t1.0.0
\n\nusage:\t./hafoe.sh [options] { -parentlib <fa> -chimericlib <csv> -enrichedlib1 <fq1> -enrichedlib2 <fq2> -exploreout <path_to_output_folder> -o <o>}
\n\nInput:
\n\n\t<fa>\tfasta file (required when --explore specified: the full path to parental sequences)
\n\n\t<csv>\tcsv file (required when --explore specified: the full path to chimeric sequences)
\n\n\t<fq1>\tfastq file (required when --explore, --identify specified: the full path to the sequences after 1st enrichment cycle)
\n\n\t<fq2>\tfastq file (required when --explore, --identify specified: the full path to the sequences after 2nd enrichment cycle)
\n\n\t<path_to_output_folder>\tfolder (required when only --identify specified: the full path to output folder generated by running hafoe with only --explore option)
...
\n\n\tEither one of --explore, --identify options should be specified, or both.
...
if [ ! -z $enrichedlib1 ]; then
	if [ ! -f $enrichedlib1 ]; then
		echo -e "$(tput setaf 1;) \nError:\tthe file <fq1> $enrichedlib1 does not exist $(tput sgr0)"
		echo -e $usage
		exit 1
	fi
fi
...
if [ ! -z $enrichedlib2 ]; then
	if [ ! -f $enrichedlib2 ]; then
		echo -e "$(tput setaf 1;) \nError:\tthe file <fq2> $enrichedlib2 does not exist $(tput sgr0)"
		echo -e $usage
		exit 1
	fi
fi
